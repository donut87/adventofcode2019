...
function createCoordinates(path) {
  let x = 0
  let y = 0
  let coordinates = []
  for (var i = 0; i < path.length; i++) {
    currentDirection = path[i].split("")[0];
    steps = path[i].slice(1);
...
function steppings(intersection, path) {
  let x = 0
  let y = 0
  let stepCount = 0
  let result = []
  for (var i = 0; i < path.length; i++) {
    ...
    for (var j = 0; j < steps; j++) {
      ...
      stepCount++
      if(intersection.includes(JSON.stringify({'x': x, 'y': y}))) {
        result[JSON.stringify({'x': x, 'y': y})] = stepCount
      }
    }
  }
  return result
...
let coordinates1 = createCoordinates(path1)
let coordinates2 = createCoordinates(path2)
let coordinates1stringified = coordinates1.map(z => JSON.stringify(z))
let coordinates2stringified = coordinates2.map(z => JSON.stringify(z))
let intersections = coordinates2stringified.filter(c => coordinates1stringified.includes(c))
let steppings1 = steppings(intersections, path1)
let steppings2 = steppings(intersections, path2)

const result = intersections.map(i => steppings1[i] + steppings2[i]).sort((a, b) => a - b)
