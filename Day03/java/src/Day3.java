...
public class Day3 {
    ...
    private static Map<Coordinate, Integer> steppings(List<Coordinate> intersections, String[] path) {
        ...
        for (int i = 0; i < path.length; i++) {
            String currentDirection = path[i].split("")[0];
            int steps = Integer.valueOf(path[i].substring(1));
            for (int j = 0; j < steps; j++) {
                ...
                stepCount++;
                if(intersections.contains(new Coordinate(x, y))) {
                    result.put(new Coordinate(x, y), stepCount);
                }
            }
        }
    ...
    private static List<Coordinate> createCoordinates(String[] path) {
        int x = 0;
        int y = 0;
        List<Coordinate> coordinates = new ArrayList<>();
        for (int i = 0; i < path.length; i++) {
